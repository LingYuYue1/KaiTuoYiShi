...
import { 创建默认剧情编织系统设置, 归一化剧情编织系统设置, type API配置项 } from '@/models/settings';
...
describe('runWorldEvolutionStep', () => {
  ...
  it('无到期事件且无线索时不调用模型', async () => {
    const result = await runWorldEvolutionStep({ config, events: [], dueInstanceIds: [], clues: [], 当前游戏日: 4 });
  ...
  it('到期事件触发调用并返回候选；仅线索也可触发', async () => {
    nonStreamMock.mockResolvedValue('[{"eventInstanceId":"a","action":"resolve"}]');
    const byDue = await runWorldEvolutionStep({ config, events: [事件('a')], dueInstanceIds: ['a'], clues: [], 当前游戏日: 4 });
    expect(byDue).toMatchObject({ ok: true, skipped: false });
    expect(nonStreamMock).toHaveBeenCalledTimes(1);

    const byClue = await runWorldEvolutionStep({ config, events: [], dueInstanceIds: [], clues: ['广场上人群聚集'], 当前游戏日: 4 });
  ...
  it('未配置 / 解析失败 / 调用异常一律非阻断失败', async () => {
    ...
    nonStreamMock.mockResolvedValue('nope');
    expect((await runWorldEvolutionStep({ ...base, config })).ok).toBe(false);
