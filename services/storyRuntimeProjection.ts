...
// dueAt = 分段时间线起点锚定的游戏日（解析失败回退下一游戏日）；
// 已有实例（无论终态）不重建，保证重投影幂等。
...
import { 解析时间线锚点日 } from '@/utils/storyTimelineAnchor';

export function 投影排期事件(
  system: 剧情编织系统,
  runtime: 剧情编织运行时,
  当前游戏日: number,
  当前日期?: string,
  ...
  const anchored = 当前日期 ? 解析时间线锚点日(next.时间线起点, 当前日期, 当前游戏日) : undefined;
  const instance: 世界事件实例 = {
    eventInstanceId,
    segmentId: next.id,
    标题: next.标题,
    dueAt: anchored ?? Math.max(1, Math.trunc(当前游戏日) + 1),
    status: 'scheduled',
    updatedAt: Date.now(),
  };
