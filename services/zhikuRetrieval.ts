...
const ZHIKU_SCENE_HINTS: Record<string, string[]> = {
  ...
  planarcadia_welcome: ['二相乐园', '幻月', '幻月游戏', '愿力', '幻造种', '花火', '真珠', '突发事件应对指南'],
  planarcadia_pigeon_river: ['二相乐园', '鸽川区', '告死魔', '共愿帮', '不死途', '虚照', '世界尽头酒馆'],
  planarcadia_academy: ['二相乐园', '绘世学院', '火花', '火花大会', '模因病毒', '认知篡改', '直播'],
  planarcadia_ink_residue: ['二相乐园', '残卷', '墨色', '绯英', '建木', '渡画泉隐', '贪饕', '归寂'],
};
...
function buildZhikuSceneHints(sceneContext?: 智库场景上下文): string[] {
  ...
  if (sceneContext.startScenarioId && sceneContext.startScenarioId in ZHIKU_SCENE_HINTS) {
  ...
  if (/二相乐园|幻月|幻造|愿力|鸽川|绘世学院|残卷|绯英|归寂/.test(raw)) {
    addHints(ZHIKU_SCENE_HINTS.planarcadia_welcome);
    addHints(ZHIKU_SCENE_HINTS.planarcadia_pigeon_river);
    addHints(ZHIKU_SCENE_HINTS.planarcadia_academy);
    addHints(ZHIKU_SCENE_HINTS.planarcadia_ink_residue);
  }
...
function getNormalRelatedLimit(limit: number): number {
  return Math.min(Math.max(1, Math.trunc(limit || NORMAL_KEYWORD_RECALL_LIMIT)), NORMAL_KEYWORD_RECALL_LIMIT);
}
...
function buildCharacterAnchorEntries(system: 智库系统, query: string, limit: number, sceneContext?: 智库场景上下文): 智库条目[] {
  ...
  const anchorLimit = getCharacterAnchorLimit();
...
function getCharacterAnchorLimit(): number {
...
export async function retrieveZhikuContextWithModel(
  ...
  try {
    ...
    const supplementGroups = parseZhikuIndexes(rawText, candidates);
...
function buildZhikuInjection(groups: 智库召回分组, sceneHints: string[] = []): string {
  ...
  return [
    '# 本回合角色档案与设定参考',
    '',
    '以下是为你调取的场景设定与在场角色资料。请将它们作为绝佳的“表演指导”和“世界观香料”，用以极大丰富本回合的演出细节。',
    '- 请将“人物性格底色”作为角色的灵魂。在描写中自然融入其外貌特征、说话方式和行为习惯，时刻保持对其“避雷区域（禁止误写）”的敬畏，确保角色原汁原味。',
    '- 请将“当前时间、地点、既成事件与关系”视为绝对的现实舞台。若提供的背景资料与当前事实存在温差，请始终以当前的互动事实为准，用资料来丰富他们在当前境遇下的独特表现，而非生搬硬套历史记录。',
    '- 对于宏大的设定资料、渊源考据，请将它们化为环境的气质底色或旁白中高雅的修辞，避免将整理者的推测当成真理强推剧情。',
    groups.characterEntries.length
      ? [
          '## 在场角色档案（性格与演技指导）',
          '- 请赋予角色灵魂：本回合中，务必让该组人物通过特有的对话、标志性的微动作或表情来展现他们的人格魅力。',
          '- 拒绝背景板：请让原著角色主动呼吸、发声和行动，展现出鲜明的个人印记，绝不要让他们沦为干瘪的名字或沉默的道具。',
          '- 灵活的情感温标：资料提供的关系是初始底色。若玩家的互动已经让关系升温或产生裂痕，请顺从当下积累的羁绊，去自然诠释他们独特的态度。',
        ].join('\n')
      : '',
    sceneHints.length ? `场景关键词索引：${sceneHints.slice(0, 8).join('、')}` : '场景关键词索引：无',
    '',
    ...formatGroup('在场角色档案（性格与演技指导）', groups.characterEntries),
    '',
    ...formatGroup('强相关背景参考', groups.strongEntries),
    '',
    ...formatGroup('弱相关背景参考', groups.weakEntries),
