...
import { isZhikuDataOwner, isZhikuGovernanceCategory } from './zhikuGovernance';
...
export function 创建智库条目(input: {
  ...
  return {
    ...
    摘要: input.摘要?.trim() ?? '',
    原文: input.原文?.trim() ?? '',
    注入内容: 归一化智库注入内容(input.注入内容, category),
...
/**
 * **边界归一化**：不可信输入（IDB 旧档、外部载荷）的修复入口——唯一允许清洗、补默认、去重兜底
 * 的地方。可信产物（内置 zhiku 预设装配结果、构建期生成物）走 构造智库系统：它们已由契约测试
 * 定型。返回值不提供私有副本：调用方不得原地修改。
 */
...
/**
 * **可信构造**：输入是已定型条目（内置预设装配结果、构建期生成物）。不逐字段清洗、不补默认、
 * 不去重兜底——重复 ID 或缺字段属于契约被破坏，由契约测试拦截而非运行时悄悄修复。
...
/** 显式触发词：优先结构化触发关键词，其次核心触发词，最后从关键词标签中提取。 */
...
export function 获取智库核心触发词(entry: Pick<智库条目, '原文'>): string[] {
  const source = typeof entry.原文 === 'string' ? entry.原文 : '';
...
function normalizeEntry(entry: Partial<智库条目>, ctx?: VariableExecContext): 智库条目 {
  ...
  return {
    ...
    来源序号: normalizeNonNegativeInteger(entry.来源序号),
...
function scoreEntry(entry: 智库条目, query: string, terms: string[]): number {
  ...
  // 关键词取标签值：正文写「符玄」匹配「角色:符玄」的值，而不是带前缀的原文。
  const keywordValues = entry.关键词
    .map((keyword) => parseKeywordTag(keyword)?.value ?? keyword)
