...
/**
 * **边界归一化**：把不可信输入（IDB 旧档、导入文本、外部载荷）修复成合法系统。
 *
 * 这是唯一允许清洗、补默认、生成 ID 的入口。可信产物（内置 canon、构建期生成物）不要走这里，
 * 走 构造剧情编织系统——它们已由构建期与契约测试定型，运行时不需要重新证明自己合法。
...
/**
 * **可信构造**：输入是构建期/测试期已定型的剧情编织系统资产（内置 canon、构建期生成物）。
 *
 * 不逐字段清洗、不补默认、不生成 ID：缺字段即契约被破坏，由契约测试拦截，而不是运行时悄悄修复。
 * 只做允许的派生（进度锚点、运行时默认）；系列级聚合由 构造剧情编织系列 负责。
 * 返回值不提供私有副本：调用方不得原地修改。引用稳定性是明确契约——
 * 内容未改变时调用方应沿用原引用，只有 IDB 覆盖或真实变更才创建新对象。
 */
...
/**
 * **可信构造单个系列**：输入是已定型产物。只重算派生聚合（核心角色/地点/派系/阶段概括）——
 * 它们跟随运行态（处理状态/运行状态/启用注入）变化，属于运行时必须的派生，不属于清洗。
 */
