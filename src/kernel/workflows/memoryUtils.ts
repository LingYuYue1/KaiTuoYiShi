...
export function autoCompressMemorySystem(
  ...
  const shortThreshold = requirePositiveInteger(settings.短期转中期阈值, 'Short memory threshold');
  const middleThreshold = requirePositiveInteger(settings.中期转长期阈值, 'Middle memory threshold');
...
export async function autoCompressMemorySystemWithArchivesAsync(
  ...
  settings: 记忆系统设置,
  signal?: AbortSignal,
): Promise<{ memory: 记忆系统; archives: 回忆条目[]; usedModel: boolean }> {
  let next = system;
  const archives: 回忆条目[] = [];
  const immediateThreshold = requirePositiveInteger(settings.即时转短期阈值, 'Immediate memory threshold');
  const shortThreshold = requirePositiveInteger(settings.短期转中期阈值, 'Short memory threshold');
  ...
  return { memory: next, archives, usedModel };
}
