...
import { buildStoryPlanningAnalysis } from '@/src/kernel/domain/story/storyPlanningAnalysis';
...
function formatMainRequestOrderOverview(
  ...
  if (tavernStatus) {
    lines.push(
      ...
      `- 酒馆消息链已启用：${tavernStatus.enabled ? '是' : '否'}`,
...
function buildMainContextSnapshot(state: TurnExecutionState): ContextSnapshot {
  ...
  const tavernStatus: Parameters<typeof formatMainRequestOrderOverview>[2] = {
    ...
    reason: tavernV2Enabled ? '' : '未选择酒馆预设，使用原生主流程。',
  };
  ...
  if (currentPresetV2) {
    ...
    tavernStatus.reason = '快照严格使用当前酒馆 V2 预设；构建失败不会切换到原生消息链。';
  }
...
function buildZhikuContextSnapshot(state: TurnExecutionState): ContextSnapshot {
  ...
  addSection(sections, {
    ...
    category: '实际',
    content: actualYitingRecallPreview || '（上一条 AI 回复没有保存忆庭召回诊断；请从新增诊断后的新回合开始查看。）',
  });
  addSection(sections, {
    id: 'zhiku_actual_saved_preview',
    title: '上一回合真实保存的召回诊断',
    category: '实际',
    content: actualZhikuRecallPreview || '（上一条 AI 回复没有保存召回诊断；请从新增诊断后的新回合开始查看。）',
  });
  addSection(sections, {
    id: 'zhiku_system',
