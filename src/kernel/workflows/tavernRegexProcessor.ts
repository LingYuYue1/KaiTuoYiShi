...
export function applyTavernOutputRegexScripts(rawText: string, rawPreset: unknown): TavernRegexApplyResult {
  ...
  for (const script of scripts) {
    ...
    if (safety.disabled) continue;
    if (!isSafeOutputCleanupCandidate(script) || safety.kind !== 'output_postprocess' || safety.risky) {
      throw new Error(`Enabled Tavern regex is not executable: ${name}: ${safety.reason}`);
    }

    const source = readFindRegex(script);
    const replace = readReplaceString(script);
    if (!source.trim()) throw new Error(`Enabled Tavern regex has empty find_regex: ${name}`);
    if (UNSAFE_REPLACEMENT_RE.test(replace)) {
      throw new Error(`Enabled Tavern regex has unsafe replacement markup: ${name}`);
    }
    ...
    if (wouldRemoveProtocolTags(before, after)) {
      throw new Error(`Enabled Tavern regex would remove project protocol tags: ${name}`);
    }
    text = after;
    applied.push(name);
  }
