...
interface Props {
  ...
  /** 手机 AI 回复用例动作：封装配置构建与生成，失败时返回空字符串（由 App 从 useGame 门面注入，片 panel-p5）。 */
...
export function PhoneModal({
  ...
  const getSeedCooldown = useCallback((seed: 主动来信种子) =>
    seed.targetType === 'group'
      ? gameSettings.手机系统.groupCooldownTurns
      : gameSettings.手机系统.contactCooldownTurns,
  [gameSettings.手机系统.contactCooldownTurns, gameSettings.手机系统.groupCooldownTurns]);
  const isSeedCoolingDown = useCallback((seed: 主动来信种子) => {
  ...
  }, [getSeedCooldown, phone.messageSeeds, turnCount]);
  ...
  }, [gameSettings.手机系统.autoGenerateSeeds, generatingSeedId, isSeedCoolingDown, pendingSeeds, phoneEnabled]);

  const [prevChats, setPrevChats] = useState(phone.chats);
  if (prevChats !== phone.chats) {
    setPrevChats(phone.chats);
  ...
  const [prevActiveApp, setPrevActiveApp] = useState(activeApp);
  if (prevActiveApp !== activeApp) {
    setPrevActiveApp(activeApp);
    setMobileView('list');
  }
  ...
  const createReplyMessages = (
    ...
    return contents.map((rawContent, index) => {
      ...
      return 创建手机消息({
        senderId: chat.type === 'private' ? contact?.id ?? chat.id : speaker.id,
        senderName: chat.type === 'private' ? contact?.name ?? chat.title : speaker.name,
  ...
  const appendPhoneLocalSummary = (
    ...
    const didFlush = (): boolean => shouldFlush;
    return didFlush() ? flushedSummary : '';
  };
  ...
  const handleSendPhoneMessage = async () => {
    ...
    try {
      ...
      await appendMessagesToChatSequentially(
        activeChat.id,
        createReplyMessages(activeChat, replyMessages, contact),
        0,
      );
  ...
  const handleGenerateSeed = async (seed: 主动来信种子) => {
    ...
    try {
      ...
      await appendMessagesToChatSequentially(
        chat.id,
        createReplyMessages(chat, replyMessages, seed.targetType === 'private' ? contact : undefined, seed.id),
        0,
      );
      const flushedSummary = appendPhoneLocalSummary(
        chat,
                    ...
                    {activeChat ? (
                      <ChatSurface
                        ...
                        groupAddCandidates={activeChat.type === 'group'
                          ...
                          : []}
                        onSend={() => void handleSendPhoneMessage()}
                        draft={draft}
...
function Avatar({ name, src }: { name: string; src?: string }) {
  return (
    ...
    >
      {src ? <img src={src} alt={name} className="h-full w-full object-cover" /> : name.at(0) ?? '?'}
      <span
