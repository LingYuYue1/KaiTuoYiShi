...
import { activeAccentSurface, activeTextColor, bodyColor, cardClip, cardSurface, faintColor, heroGridRailledStyle, heroSurfaceRailled, imageWellSurface, mutedColor, smallClip, titleColor } from './visualTokens';
...
function ImageTaskCard({ item, onSelect, onPreview }: { item: Extract<FeedItem, { kind: 'image' }>; onSelect: (id: string) => void; onPreview: (value: { src: string; title: string }) => void }) {
  ...
  return <article className="overflow-hidden" style={{ background: cardSurface, border: '1px solid rgba(var(--tj-btn-primary-start), var(--tj-edge-tint-weak))', clipPath: cardClip }}><button type="button" onClick={() => { onSelect(item.entry.id); if (item.src) onPreview({ src: item.src, title: item.entry.title }); }} className="group block w-full text-left"><div className="aspect-[4/3]" style={{ background: imageWellSurface }}><SafeAlbumImage src={item.src} alt={item.entry.title} className="h-full w-full object-cover transition-transform group-hover:scale-[1.03]" emptyLabel="无图片" failedLabel="图片失效" /></div></button><div className="space-y-2 px-3 py-3"><div className="flex items-start justify-between gap-2"><div className="min-w-0"><div className="truncate font-serif text-sm font-bold tracking-[0.08em]" style={{ color: titleColor }}>{item.entry.title}</div><div className="mt-1 text-[11px]" style={{ color: faintColor }}>{slotLabel(item.entry.slot)} · {formatGenerationDate(item.entry.createdAt)}</div></div><span className="shrink-0 px-2 py-1 text-[10px]" style={{ color: tone.color, background: tone.background, border: `1px solid ${tone.border}`, clipPath: smallClip }}>{historyKindLabel(imageKind)}</span></div><div className="text-[11px]" style={{ color: 'rgba(var(--tj-btn-primary-start),0.8)' }}>{source}</div>{item.task && <details><summary className="cursor-pointer text-[11px] tracking-[0.1em]" style={{ color: mutedColor }}>生成详情</summary><div className="mt-2 space-y-2 text-xs" style={{ color: mutedColor }}><div>{imageBackendLabel(item.task.backend)} · 重试 {item.task.retryCount} 次</div>{item.task.finalPrompt || item.task.prompt ? <PromptBlock title="正向提示词" text={item.task.finalPrompt || item.task.prompt} /> : null}</div></details>}</div></article>;
}
...
function PendingTaskCard({ item, onRetry }: { item: Extract<FeedItem, { kind: 'task' }>; onRetry: (task?: 图片生成任务) => void }) {
  ...
  return <article className="flex min-h-[210px] flex-col justify-between px-4 py-4" style={{ background: cardSurface, border: `1px solid ${item.orphaned ? 'rgba(var(--tj-tech-cyan), var(--tj-edge-tint-strong))' : tone.border}`, clipPath: cardClip }}><div><div className="flex items-start justify-between gap-2"><div className="min-w-0"><div className="truncate font-serif text-sm font-bold tracking-[0.08em]" style={{ color: titleColor }}>{taskPromptTitle(item.task)}</div><div className="mt-1 text-[11px]" style={{ color: mutedColor }}>{slotLabel(item.task.slot)} · {imageBackendLabel(item.task.backend)}</div></div><span className="shrink-0 px-2 py-1 text-[10px]" style={{ color: item.orphaned ? 'rgba(var(--tj-tech-cyan),0.94)' : tone.color, background: item.orphaned ? 'rgba(var(--tj-tech-cyan),0.08)' : tone.background, border: `1px solid ${item.orphaned ? 'rgba(var(--tj-tech-cyan), var(--tj-edge-tint))' : tone.border}`, clipPath: smallClip }}>{label}</span></div><div className="mt-4 text-xs leading-relaxed" style={{ color: item.orphaned ? 'rgba(var(--tj-tech-cyan),0.9)' : mutedColor }}>{item.orphaned ? '任务已成功结束，但图片未进入相册，记录保留供排查。' : item.task.error || `创建于 ${formatGenerationDate(item.task.createdAt)}`}</div></div><div className="mt-4 space-y-2">{item.task.status === 'failed' && <button type="button" onClick={() => onRetry(item.task)} className="w-full px-3 py-2 font-serif text-xs tracking-[0.14em]" style={{ color: 'rgba(var(--tj-btn-primary-start),0.94)', background: 'rgba(var(--tj-btn-primary-start),0.075)', border: '1px solid rgba(var(--tj-btn-primary-start), var(--tj-edge-tint))', clipPath: smallClip }}>重试</button>}<details><summary className="cursor-pointer text-[11px] tracking-[0.1em]" style={{ color: mutedColor }}>任务详情</summary><div className="mt-2 text-xs" style={{ color: mutedColor }}>来源：{generationSourceLabel(item.task.source)} · 重试：{item.task.retryCount} 次</div></details></div></article>;
}

function TaskFeedEmpty({ filter }: { filter: TaskFeedFilter }) {
  return <div className="px-4 py-16 text-center" style={{ color: faintColor, background: 'rgba(var(--tj-ui-panel),0.42)', border: '1px solid rgba(var(--tj-btn-primary-start), var(--tj-edge-tint-weak))', clipPath: cardClip }}><div className="font-serif text-sm tracking-[0.24em]">{filter === 'all' ? '暂无图片任务' : '当前筛选下暂无记录'}</div><div className="mt-2 text-xs">生成或导入图片后，会在这里按时间汇总。</div></div>;
}
