      ...
      <div className="flex h-full min-h-0 flex-col gap-4">
        <Panel title="图片工具" className="shrink-0" contentClassName="space-y-3">
          ...
          <div className="grid gap-2 sm:grid-cols-4">
            ...
            <GalleryButton onClick={() => { setBatchMode((current) => !current); setSelectedIds([]); }} active={batchMode}>{batchMode ? `批量选择中 · ${selectedVisibleIds.length}` : '批量选择'}</GalleryButton>
            <GalleryButton disabled={!batchMode || selectedVisibleIds.length === 0} danger onClick={() => onDeleteEntries(selectedVisibleIds)}>批量删除</GalleryButton>
            {scope === 'character' ? <GalleryButton disabled={!activeRecord || !activeItemId} primary onClick={() => setSlotPickerOpen(true)}>设置到槽位</GalleryButton> : <div />}
          </div>
        </Panel>
...
function LibraryImportDialog({ open, onClose, scope, record, traveler, onImport }: { open: boolean; onClose: () => void; scope: GalleryScope; record: CharacterLibraryRecord | null; traveler: 角色数据结构; onImport: (file: File | null, target: AlbumImportTarget) => void }) {
  ...
  const handleFile = (file: File | null) => {
    if (!file) return;
    if (!window.confirm(`导入会替换当前相册。确认将文件导入到“${targetLabel}”吗？`)) return;
    onImport(file, target);
    onClose();
  };
  return createPortal(<div className="fixed inset-0 z-[130] flex items-center justify-center bg-black/70 p-4" onMouseDown={onClose}><div className="w-full max-w-md px-4 py-4" style={{ background: 'linear-gradient(160deg, rgba(var(--tj-surface),0.98), rgba(var(--tj-bg-primary),0.98))', boxShadow: 'inset 0 0 0 1px rgba(var(--tj-btn-primary-start),0.28)', clipPath: cardClip }} onMouseDown={(event) => event.stopPropagation()}><div className="font-serif text-sm font-bold tracking-[0.18em]" style={{ color: 'rgb(var(--tj-ui-title))' }}>导入到当前图库</div><div className="mt-3 space-y-2 text-xs leading-relaxed" style={{ color: 'rgba(var(--tj-ui-muted),0.78)' }}><div>归类目标：{targetLabel}</div><div style={{ color: 'rgba(var(--tj-danger),0.92)' }}>导入会替换当前相册中的全部资源与任务，建议先导出备份。</div></div><div className="mt-4 flex justify-end gap-2"><button type="button" onClick={onClose} className="px-3 py-2 text-xs" style={{ color: 'rgba(var(--tj-ui-muted),0.82)' }}>取消</button><button type="button" onClick={() => fileRef.current?.click()} className="px-3 py-2 font-serif text-xs tracking-[0.14em]" style={{ color: 'rgb(var(--tj-ui-active-text))', background: activeAccentSurface, clipPath: smallClip }}>选择相册文件</button></div><input ref={fileRef} type="file" accept="application/json,.json" className="hidden" onChange={(event) => handleFile(event.target.files?.[0] ?? null)} /></div></div>, document.body);
}
