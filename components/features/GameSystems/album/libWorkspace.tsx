        ...
        <div className="flex h-full min-h-0 flex-col gap-3">
          <div className="space-y-1.5">
            {filterOptions.map((option) => {
              ...
              return <button key={option.id} type="button" onClick={() => { setScope(option.id); setBatchMode(false); setSelectedIds([]); }} className="flex w-full items-center justify-between gap-2 px-3 py-2.5 text-left" style={{ color: active ? 'rgb(var(--tj-ui-title))' : 'rgba(var(--tj-ui-muted),0.82)', background: active ? 'linear-gradient(90deg, rgba(var(--tj-btn-primary-start),0.16), rgba(var(--tj-tech-cyan),0.05))' : 'rgba(var(--tj-ui-panel-strong),0.36)', border: active ? '1px solid rgba(var(--tj-btn-primary-start), var(--tj-edge-tint-strong))' : '1px solid rgba(var(--tj-btn-primary-start), var(--tj-edge-tint-weak))', clipPath: smallClip }}><span className="font-serif text-sm font-bold tracking-[0.14em]">{option.label}</span><span className="text-[11px]" style={{ color: 'rgba(var(--tj-ui-muted),0.62)' }}>{option.count}</span></button>;
            })}
          </div>
        ...
        <Panel title="图片工具" className="shrink-0" contentClassName="space-y-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex gap-2">
              <button type="button" disabled={operationBusy} onClick={() => setImportOpen(true)} className="px-3 py-2 text-xs font-serif tracking-[0.14em] disabled:opacity-45" style={{ color: 'rgba(var(--tj-btn-primary-start),0.9)', background: 'rgba(var(--tj-btn-primary-start),0.055)', border: '1px solid rgba(var(--tj-btn-primary-start), var(--tj-edge-tint))', clipPath: smallClip }}>导入</button>
              <button type="button" disabled={operationBusy} onClick={onExport} className="px-3 py-2 text-xs font-serif tracking-[0.14em] disabled:opacity-45" style={{ color: 'rgba(var(--tj-tech-cyan),0.92)', background: 'rgba(var(--tj-tech-cyan),0.06)', border: '1px solid rgba(var(--tj-tech-cyan), var(--tj-edge-tint))', clipPath: smallClip }}>导出</button>
...
function GalleryButton({ children, onClick, disabled = false, active = false, danger = false, primary = false }: { children: ReactNode; onClick: () => void; disabled?: boolean; active?: boolean; danger?: boolean; primary?: boolean }) {
  ...
  return <button type="button" disabled={disabled} onClick={onClick} className="min-h-[42px] px-3 py-2 font-serif text-xs tracking-[0.14em] disabled:opacity-45" style={{ color, background, border: danger ? '1px solid rgba(var(--tj-danger), var(--tj-edge-tint))' : '1px solid rgba(var(--tj-btn-primary-start), var(--tj-edge-tint))', clipPath: smallClip }}>{children}</button>;
}

function CharacterRecordButton({ record, active, onClick }: { record: CharacterLibraryRecord; active: boolean; onClick: () => void }) {
  return <button type="button" onClick={onClick} className="block w-full min-w-0 px-3 py-2.5 text-left" style={{ background: active ? 'linear-gradient(90deg, rgba(var(--tj-btn-primary-start),0.16), rgba(var(--tj-btn-primary-start),0.04))' : 'rgba(var(--tj-ui-panel-strong),0.36)', border: active ? '1px solid rgba(var(--tj-btn-primary-start), var(--tj-edge))' : '1px solid rgba(var(--tj-btn-primary-start), var(--tj-edge-tint-weak))', clipPath: smallClip }}><div className="truncate font-serif text-sm font-bold tracking-[0.1em]" style={{ color: 'rgb(var(--tj-ui-title))' }}>{record.name}</div><div className="mt-1 flex justify-between gap-2 text-[11px]" style={{ color: 'rgba(var(--tj-ui-muted),0.66)' }}><span>已装 {record.mountedCount}</span><span>资源 {record.resourceCount}</span></div></button>;
}
...
function CharacterGalleryCard({ item, active, batchMode, selected, reference, onClick }: { item: CharacterLibraryEntry; active: boolean; batchMode: boolean; selected: boolean; reference: boolean; onClick: () => void }) {
  return <div className="relative overflow-hidden" style={{ contentVisibility: 'auto', containIntrinsicSize: '240px', background: selected ? 'rgba(var(--tj-tech-cyan),0.12)' : 'rgba(var(--tj-ui-panel),0.52)', border: active || selected ? '1px solid rgba(var(--tj-btn-primary-start), var(--tj-edge-strong))' : item.entry.nsfw ? '1px solid rgba(var(--tj-ui-nsfw), var(--tj-edge-tint-strong))' : '1px solid rgba(var(--tj-btn-primary-start), var(--tj-edge-tint-weak))', clipPath: cardClip }}><button type="button" onClick={onClick} className="group block w-full text-left"><div className="relative aspect-[4/3]"><SafeAlbumImage src={item.src} alt={item.entry.title} className="h-full w-full object-cover transition-transform group-hover:scale-[1.03]" emptyLabel="无图片" failedLabel="图片失效" />{reference && <span className="absolute left-2 top-2 px-2 py-1 font-serif text-[10px] tracking-[0.12em]" style={{ color: 'rgba(var(--tj-tech-cyan),0.96)', background: 'rgba(0,0,0,0.62)', border: '1px solid rgba(var(--tj-tech-cyan), var(--tj-edge-tint-strong))', clipPath: smallClip }}>参考图</span>}</div><div className="space-y-1 px-3 py-2"><div className="flex items-center justify-between gap-2"><span className="truncate font-serif text-sm" style={{ color: 'rgb(var(--tj-ui-title))' }}>{item.entry.title}</span>{batchMode && <span className="shrink-0 text-[10px]" style={{ color: selected ? 'rgba(var(--tj-tech-cyan),0.96)' : 'rgba(var(--tj-ui-muted),0.6)' }}>{selected ? '已选择' : '点选'}</span>}</div><div className="flex justify-between gap-2 text-[11px]" style={{ color: 'rgba(var(--tj-ui-muted),0.66)' }}><span>{slotLabel(item.entry.slot)}</span>{item.entry.nsfw && <span style={{ color: 'rgb(var(--tj-ui-nsfw))' }}>NSFW</span>}</div></div></button></div>;
}
...
function LibraryImportDialog({ open, onClose, scope, record, traveler, onImport }: { open: boolean; onClose: () => void; scope: GalleryScope; record: CharacterLibraryRecord | null; traveler: 角色数据结构; onImport: (file: File | null, target: AlbumImportTarget, mode: AlbumImportMode) => void }) {
  ...
  return createPortal(<div className="fixed inset-0 z-[130] flex items-center justify-center bg-black/70 p-4" onMouseDown={onClose}><div className="w-full max-w-md px-4 py-4" style={{ background: 'linear-gradient(160deg, rgba(var(--tj-surface),0.98), rgba(var(--tj-bg-primary),0.98))', border: '1px solid rgba(var(--tj-btn-primary-start), var(--tj-edge-tint))', clipPath: cardClip }} onMouseDown={(event) => event.stopPropagation()}><div className="font-serif text-sm font-bold tracking-[0.18em]" style={{ color: 'rgb(var(--tj-ui-title))' }}>导入相册文件</div><div className="mt-3 space-y-2 text-xs leading-relaxed" style={{ color: 'rgba(var(--tj-ui-muted),0.78)' }}><div>合并归类目标：{targetLabel}</div><div>支持本项目导出的 ZIP 备份和旧版相册 JSON。合并导入会保留当前内容并自动复用相同图片。</div><div style={{ color: 'rgba(var(--tj-danger),0.88)' }}>“覆盖恢复”仅用于完整备份，会替换当前相册中的全部资源与任务。</div></div><div className="mt-4 grid gap-2 sm:grid-cols-[auto_1fr_1fr]"><button type="button" onClick={onClose} className="px-3 py-2 text-xs" style={{ color: 'rgba(var(--tj-ui-muted),0.82)' }}>取消</button><button type="button" onClick={() => chooseFile('replace')} className="px-3 py-2 font-serif text-xs tracking-[0.12em]" style={{ color: 'rgba(var(--tj-danger),0.92)', background: 'rgba(var(--tj-danger),0.09)', border: '1px solid rgba(var(--tj-danger), var(--tj-edge-tint))', clipPath: smallClip }}>覆盖恢复</button><button type="button" onClick={() => chooseFile('merge')} className="px-3 py-2 font-serif text-xs tracking-[0.12em]" style={{ color: 'rgb(var(--tj-ui-active-text))', background: activeAccentSurface, clipPath: smallClip }}>合并导入</button></div><input ref={fileRef} type="file" accept="application/zip,.zip,application/json,.json" className="hidden" onChange={(event) => { const input = event.currentTarget; handleFile(input.files?.[0] ?? null); input.value = ''; }} /></div></div>, document.body);
}
