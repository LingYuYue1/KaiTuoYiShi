...
export function MemorySystemPanel({ memorySystem, onCompress, settings }: MemorySystemPanelProps) {
  ...
  const compressLayer = async (layer: Exclude<MemoryLayer, 'long'>) => {
    if (compressingLayer) throw new Error(`Memory compression already running for ${compressingLayer}`);
        ...
        <div className="min-h-full px-3 py-3 md:px-5 md:py-5" style={panelStyle}>
          <div className="flex flex-wrap items-end justify-between gap-3">
            ...
            <div className="flex flex-wrap gap-2">
              {activeLayer === 'immediate' && (
                <ActionButton disabled={compressingLayer !== null} onClick={() => void compressLayer('immediate')}>
                  {compressingLayer === 'immediate' ? '压缩中…' : '压缩到短期'}
                </ActionButton>
              )}
              {activeLayer === 'short' && (
                <ActionButton disabled={compressingLayer !== null} onClick={() => void compressLayer('short')}>
                  {compressingLayer === 'short' ? '压缩中…' : '压缩到中期'}
                </ActionButton>
              )}
              {activeLayer === 'middle' && (
                <ActionButton disabled={compressingLayer !== null} onClick={() => void compressLayer('middle')}>
                  {compressingLayer === 'middle' ? '压缩中…' : '压缩到长期'}
                </ActionButton>
              )}
            </div>
          </div>
          ...
          {compressionError && (
            <div className="mt-3 px-3 py-2 text-sm" style={{ color: 'rgba(var(--tj-danger),0.92)', boxShadow: 'inset 0 0 0 1px rgba(var(--tj-danger),0.28)', clipPath: smallClip }}>
              {compressionError}
            </div>
          )}
...
function ActionButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className="font-serif text-[12px] tracking-[0.18em] px-3 py-1.5 transition-all hover:bg-[rgba(var(--tj-accent-primary),0.08)] disabled:cursor-not-allowed disabled:opacity-50"
      style={{
