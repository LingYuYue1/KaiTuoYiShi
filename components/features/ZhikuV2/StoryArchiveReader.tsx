...
const STORY_CATEGORY = ZHIKU_DESIGN_CATEGORIES.find((category) => category.id === 'story')!;
...
export function StoryArchiveReader({
  ...
  return (
    <section
      className="zhiku-v2-browser zhiku-v2-story-reader"
    ...
    >
      ...
      <span className="zhiku-v2-story-reader__watermark" style={watermarkStyle} aria-hidden="true" />
      <div className="zhiku-v2-header__actions zhiku-v2-story-reader__close">
        <button type="button" onClick={onClose} aria-label="关闭智库" title="关闭智库">
          <X size={23} strokeWidth={1.5} />
        </button>
      </div>

      <main className="zhiku-v2-story-reader__content" data-mobile-pane={mobilePane}>
        <aside className="zhiku-v2-story-reader__catalog" aria-label="剧情卷宗与章节目录">
          <header>
            <div className="zhiku-v2-story-reader__identity">
              {onBack && (
                <button
                  type="button"
                  className="zhiku-v2-header__back"
                  onClick={onBack}
                  aria-label="返回分类大厅"
                  title="返回分类大厅"
                >
              ...
              <span className="zhiku-v2-header__mark" data-emblem="true" style={emblemStyle} aria-hidden="true">
                <span />
              </span>
              <div className="zhiku-v2-story-reader__catalog-title">
                <h1>剧情档案</h1>
                <p>卷宗目录</p>
              </div>
            </div>
          ...
          <div className="zhiku-v2-story-reader__archive-list">
            {volumes.map((volume) => {
              ...
              return (
                <section
                  key={volume.id}
                  className="zhiku-v2-story-reader__archive-group"
                  data-active={activeVolume ? 'true' : 'false'}
                  data-locked={volumeLocked ? 'true' : 'false'}
                >
                  <button
                    type="button"
                    className="zhiku-v2-story-reader__archive-button"
                  ...
                  >
                    <span className="zhiku-v2-story-reader__archive-index">
                      {volumeLocked ? <Lock size={13} aria-hidden="true" /> : <FileText size={14} aria-hidden="true" />}
                      {volume.number}
                    </span>
                    <span className="zhiku-v2-story-reader__archive-copy">
                      ...
                      {volume.chapters.map((chapter) => {
                        ...
                        return (
                          <li key={chapter.id}>
                            ...
                            >
                              <span className="zhiku-v2-story-reader__chapter-number">{chapter.number}</span>
                              <span className="zhiku-v2-story-reader__chapter-copy">
                                <strong>{locked ? '未解锁章节' : chapter.title}</strong>
                                <small>{locked ? '故事仍在远方' : chapter.subtitle ?? '章节档案'}</small>
                              </span>
                              <span className="zhiku-v2-story-reader__chapter-status">
          ...
          <footer className="zhiku-v2-story-reader__catalog-progress" style={progressStyle}>
            <span>当前卷阅读</span>
        ...
        <section className="zhiku-v2-story-reader__detail" style={readerStyle} aria-live="polite">
          ...
          <button
            type="button"
            className="zhiku-v2-story-reader__mobile-catalog-trigger"
          ...
          {selectedChapter ? (
            <>
              <header className="zhiku-v2-story-reader__chapter-heading">
                <span>STORY ARCHIVE // {selectedVolume?.number ?? '--'}</span>
                <h2>{selectedVolume?.title}</h2>
                <div className="zhiku-v2-story-reader__chapter-title">
                  <b>{selectedChapter.number}</b>
                  <h3>{selectedChapter.title}</h3>
                </div>
                {selectedChapter.summary && <p>{selectedChapter.summary}</p>}
                <div className="zhiku-v2-story-reader__chapter-meta">
              ...
              <article ref={readingPaneRef} className="zhiku-v2-story-reader__reading-pane" tabIndex={0}>
                <div className="zhiku-v2-story-reader__prose">
                  {selectedChapter.body.trim().split(/\n\s*\n/).filter(Boolean).map((paragraph, index) => (
                    <p key={`${selectedChapter.id}-${index}`}>{paragraph.replace(/\n/g, ' ')}</p>
                  ))}

                  <div className="zhiku-v2-story-reader__chapter-end" aria-hidden="true">
                    <i />
                    <span>{selectedChapter.number} · END</span>
                    <i />
                  </div>

                  <nav className="zhiku-v2-story-reader__chapter-nav" aria-label="章节切换">
                    <button
                      type="button"
            ...
            <div className="zhiku-v2-story-reader__empty">
