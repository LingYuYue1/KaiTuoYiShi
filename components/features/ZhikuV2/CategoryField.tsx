...
import type { ZhikuDesignCategory, ZhikuNodePlacement } from './types';
...
const ZHIKU_NODE_REVEAL_ORDER: Record<ZhikuDesignCategory['id'], number> = {
  character: 0,
  story: 1,
  aeon: 2,
  path: 3,
...
export function CategoryNodeSlot({
  ...
  return <div className="zhiku-v2-field__slot" data-reveal-order={revealOrder} style={style}>{children}</div>;
}
  ...
  return (
    <div className="zhiku-v2-field">
      {categories.map((category) => {
        ...
        if (renderNode) {
          return (
            <div key={category.id} className="zhiku-v2-field__render-slot">
