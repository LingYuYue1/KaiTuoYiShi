...
export function DataOrbitLayer({
  ...
  return (
    <svg
      className={`zhiku-v2-orbits zhiku-v2-orbits--${layoutVariant}`}
      viewBox="0 0 100 100"
    ...
    >
      <ellipse
        className="zhiku-v2-orbits__ring"
      ...
      />
      <ellipse
        className="zhiku-v2-orbits__ring zhiku-v2-orbits__ring--wide"
        cx={mobileLayout ? 50 : 52}
        cy={mobileLayout ? 53 : 50}
      ...
      {routes.map(({ node, pathData, pathId }) => {
        ...
        return (
          <g key={node.id} className="zhiku-v2-orbits__route" data-active={active ? 'true' : 'false'}>
      ...
      {entering && !reducedMotion && (
        <g className="zhiku-v2-orbits__packet-layer">
          {routes.flatMap(({ node, order, pathId }) =>
            ['01', '10', '11'].map((glyph, packetIndex) => {
              const begin = 210 + order * 46 + packetIndex * 72;
              return (
                <g key={`${node.id}-${glyph}-${packetIndex}`} className="zhiku-v2-orbits__packet" opacity="0">
                  <text className="zhiku-v2-orbits__packet-glyph" x="-1.35" y="0.5" transform="scale(0.56 1)">
                    {glyph}
                  </text>
                  <animateMotion
                    begin={`${begin}ms`}
