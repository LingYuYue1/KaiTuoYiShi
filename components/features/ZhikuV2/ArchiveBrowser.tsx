...
export function ArchiveBrowser({
  ...
  return (
    <section className="zhiku-v2-browser" data-reduced-motion={reducedMotion ? 'true' : 'false'}>
      <ZhikuPageFrame brightness={0.68} dimmer={0.36} />
      <span className="zhiku-v2-browser__watermark" style={watermarkStyle} aria-hidden="true" />
      <div className="zhiku-v2-header__actions zhiku-v2-browser__close">
        <button type="button" onClick={onClose} aria-label="关闭智库" title="关闭智库">
          <X size={23} strokeWidth={1.5} />
        </button>
      </div>

      <main className="zhiku-v2-browser__content" data-mobile-pane={mobilePane}>
        <aside className="zhiku-v2-browser__catalog" aria-label={`${category.label}列表`}>
          <header>
            <div className="zhiku-v2-browser__catalog-identity">
              {onBack && (
                <button
                  type="button"
              ...
              <div className="zhiku-v2-browser__catalog-title">
            ...
            {items.map((item, index) => {
              ...
              return (
                <li key={item.id}>
                  ...
                  >
                    <span className="zhiku-v2-browser__sequence">{formatSequence(index)}</span>
                    {showItemVisual && (
                    ...
                    <span className="zhiku-v2-browser__catalog-copy">
        ...
        <section className="zhiku-v2-browser__detail" style={readerStyle} aria-live="polite">
          ...
          <button
            type="button"
            className="zhiku-v2-browser__mobile-catalog-trigger"
          ...
          {selectedItem ? (
            <>
              <header className="zhiku-v2-browser__entry-heading">
                ...
                <div className="zhiku-v2-browser__entry-meta">
                  <span>{selectedItem.meta ?? '已收录档案'}</span>
                  {selectedItem.status === 'new' && <b>新</b>}
                </div>
                <div
                  className="zhiku-v2-browser__view-tabs"
                  role="tablist"
              ...
              {activeView === 'archive' ? (
                <article
                  id={panelId}
                  className="zhiku-v2-browser__document"
                  role="tabpanel"
                ...
                <article
                  id={panelId}
                  className="zhiku-v2-browser__document zhiku-v2-browser__document--injection"
                  role="tabpanel"
                  aria-labelledby={injectionTabId}
                  tabIndex={0}
                >
                  {injectionVariants.length > 1 && (
                    <div className="zhiku-v2-browser__form-preview" role="tablist" aria-label="选择注入形态">
                      <span>形态预览</span>
                      <div>
                  ...
                  <section className="zhiku-v2-browser__injection-section">
                    <header className="zhiku-v2-browser__injection-heading">
                    ...
                    {secondaryKeywordLogic && secondaryKeywords.length > 0 && (
                      <p className="zhiku-v2-browser__keyword-rule">
                        <span>召回规则</span>
                        <strong>{SECONDARY_KEYWORD_LOGIC_LABELS[secondaryKeywordLogic]}</strong>
                      </p>
                    )}
                    {displayKeywords.length ? (
                      <ol className="zhiku-v2-browser__trigger-keywords">
                      ...
                      <p className="zhiku-v2-browser__injection-empty">暂无触发关键词</p>
                    )}
                  </section>
                  <section className="zhiku-v2-browser__injection-section">
                    <header className="zhiku-v2-browser__injection-heading">
                    ...
                    {injectionPreview ? (
                      <pre className="zhiku-v2-browser__injection-payload">{injectionPreview}</pre>
                    ) : (
                      <p className="zhiku-v2-browser__injection-empty">暂无可注入内容</p>
                    )}
                  </section>
                </article>
              )}
            ...
            <div className="zhiku-v2-browser__empty">
