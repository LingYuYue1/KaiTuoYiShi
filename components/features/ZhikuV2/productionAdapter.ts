...
export type ZhikuBrowsableCategoryId = Exclude<ZhikuDesignCategoryId, 'story'>;
...
export function resolveZhikuV2Category(entry: 智库条目): ZhikuBrowsableCategoryId | null {
...
function toArchiveItem(
  ...
  const category = ZHIKU_DESIGN_CATEGORIES.find((item) => item.id === categoryId);
  return {
    id: entry.id,
    title,
...
export function buildZhikuArchiveItems(system: 智库系统): ZhikuArchiveItemsByCategory {
  ...
  for (const entry of system.条目 ?? []) {
    ...
    const categoryId = resolveZhikuV2Category(entry);
...
export function buildZhikuProductionData(
  ...
  const categories = ZHIKU_DESIGN_CATEGORIES.map((category) => ({
    ...category,
