...
assert(cloudModal.includes('cloud.replaceLocalFromCloud(') && rootCapabilities.includes('catalog.replaceAllSaves(downloaded)'), 'GitHub 云存档下载到本地必须通过 cloud capability 覆盖本地存档列表。');
...
assert(rootCapabilities.includes('catalog.getSaveList()'), 'GitHub 云存档批量上传必须通过 cloud capability 读取本地存档列表。');
assert(rootCapabilities.includes('cloud.uploadAllSavesToGitHubCloud('), 'GitHub 云存档默认上传必须由 cloud capability 调用整批同步服务。');
assert(cloudModal.includes('CloudProgress'), 'GitHub 云存档上传/下载必须显示进度条。');
assert(cloudModal.includes('cloud.updateConfiguration(') && rootCapabilities.includes("preferences.set('githubCloudSaveConfig'"), 'GitHub 云存档配置必须通过 cloud capability 保存到本地设置。');
assert(cloudModal.includes('cloud.configurationForEditor()') && rootCapabilities.includes("preferences.get<") && rootCapabilities.includes("'githubCloudSaveConfig'"), 'GitHub 云存档配置必须通过 cloud capability 从本地设置读取。');
