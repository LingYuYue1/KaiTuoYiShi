...
for (const character of characters) {
  ...
  for (const variant of character.variants) {
    if (!variant.startsWith('static:')) {
      errors.push(`${character.id} must use a remote static avatar reference: ${variant}`);
      continue;
    }
    const logicalId = variant.slice('static:'.length);
    const match = /^avatar:([^:]+):(\d+)$/.exec(logicalId);
    if (!match || match[1] !== character.id) {
      errors.push(`${character.id} has an invalid static variant: ${variant}`);
      continue;
    }
    const candidateId = `${match[1]}-${match[2]}`;
    if (logicalIds.has(logicalId)) errors.push(`duplicate avatar logical id: ${logicalId}`);
    logicalIds.add(logicalId);

    if (!staticManifest.assets[logicalId]) {
      errors.push(`${logicalId} is missing from data/staticAssetManifest.json`);
    }
    if (!avatarTs.includes(`src: avatarSource('${candidateId}')`)) {
      errors.push(`builtinAvatars.ts missing ${candidateId} source`);
    }
    if (!avatarTs.includes(`reference: avatarReference('${candidateId}')`)) {
      errors.push(`builtinAvatars.ts missing ${candidateId} logical reference`);
    }
    const variantNumber = candidateId.match(/-(\d+)$/)?.[1];
