// G1.3.2.8 证据生成器（允许命名：story-runtime-g1.3.2.8-*；不是隐藏 runner）。
// - 直接运行白名单内明确列出的脚本：本包 3 条专项（evidence-index 单独最后执行）+ G1.3.2.7 6 条 +
//   G1.3.2.6 5 条（**含其 evidence-index**，上一包漏跑的门禁本包补跑）+ G1.3.2.5 7 条 + G1.3.2.4 6 条 +
//   G1.3.2.3 5 条 + G1.3.2.2 7 条 + G1.3.2.1 6 条 + G1.3.2 §9.1/9.2/9.3 31 条 + tsc + git diff --check；
// - 命令总数由最终唯一命令清单程序化计算（manifest.meta.totalCommands），summary/manifest/报告精确相等；
// - 每条记录 command、cwd、开始/结束 ISO 时间、exit code、stdout/stderr；
// - 生成 frozen-hashes / formal-import-scan / tmp-check 证据；
// - detached manifest（排除自身；所有日志落盘后；meta.totalCommands 程序化值）；
...
const groups = [
  ['G1.3.2.8 special (3, evidence-index 单独最后执行)', [
  ...
  ['G1.3.2.6 special (5, 含 evidence-index——上一包漏跑的本包补跑)', [
...
// P0-4（G1.3.2.8）：命令总数由最终唯一命令清单程序化计算。
// 命令 = 各组脚本数 + tsc 1 + git diff --check 1（evidence-index 是验证者，不计入被验证命令清单）。
const TOTAL_COMMANDS = groups.reduce((n, [, s]) => n + s.length, 0) + 1 + 1;
summary.push('== TOTAL_COMMANDS=' + TOTAL_COMMANDS + '（程序化计算：各组脚本 + tsc + diff；evidence-index 为验证者不计入）==');
