...
assert(dbService.includes("replaceAllSaves(restoredSaves, 'before-restore')"), 'desktop mirror recovery must back up current saves through the replacement transaction');
...
assert(dbService.includes("replaceAllSaves(restoredSaves, 'before-restore')"), 'desktop mirror recovery must reuse the save replacement path after asset restoration');
assert(dbService.indexOf('const restoredSaves = await restoreDesktopAssetPayloadForSavesSafely(mirroredSaves)') < dbService.indexOf("replaceAllSaves(restoredSaves, 'before-restore')"), 'desktop mirror recovery must restore payloads before replaceAllSaves');
...
assert(dbService.indexOf('const restoredSaves = await restoreDesktopAssetPayloadForSavesSafely(backup.saves)') < dbService.lastIndexOf("replaceAllSaves(restoredSaves, 'before-restore')"), 'desktop backup recovery must restore payloads before the guarded replacement transaction');
...
assert(dbService.includes("desktopBackupReason: DesktopSaveBackupReason = 'before-replace'"), 'bulk save replacement must create a desktop backup by default');
...
assert(storageManager.includes('saves.cleanupDesktopAssets()'), 'storage manager must expose desktop asset cleanup through saves capability');
assert(storageManager.includes('repairDesktopSaveMirrorIndex'), 'storage manager must repair the desktop save mirror index');
assert(storageManager.includes("saves.backupDesktop('before-repair')"), 'storage manager must back up current saves before repairing desktop mirror indexes');
...
assert(storageManager.includes('host.listDesktopSpecialSettingMirrors()'), 'storage manager must refresh dedicated setting mirror status when exporting diagnostics');
assert(storageManager.includes('host.inspectDesktopSaveMirrorHealth()'), 'storage manager must inspect save mirror health when exporting diagnostics');
assert(storageManager.includes('host.inspectDesktopSaveDeltaMirrorHealth()'), 'storage manager must inspect save delta mirror health when exporting diagnostics');
assert(storageManager.includes('host.inspectDesktopAssetMirrorHealth()'), 'storage manager must inspect asset mirror health when exporting diagnostics');
...
assert(storageManager.includes("saves.backupDesktop('manual')"), 'storage manager must expose manual desktop backups through root capabilities');
assert(storageManager.includes('saves.backupDesktopMigration()'), 'storage manager must expose one-time migration backups through root capabilities');
...
assert(storageManager.includes('saves.previewDesktopMigrationBackup()'), 'storage manager must preview one-time migration backup size through root capabilities before writing it');
...
assert(storageManager.includes('saves.restoreDesktopBackup(backup.path)'), 'storage manager must expose desktop backup recovery through root capabilities');
...
assert(storageManager.includes('saves.restoreDesktopMirror()'), 'storage manager must expose desktop mirror recovery through root capabilities');
