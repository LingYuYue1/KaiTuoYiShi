...
assert(albumPanel.includes('const mountedSrc = entry ? 创建相册资源引用(entry.assetId) : params.src'), '从成品库挂载图片时必须写入 asset 引用，不能把 dataUrl 直接塞进变量。');
assert(albumPanel.includes('挂载旅人图片(prev, { slot: mapImageSlotToTravelerSlot(params.slot), src: mountedSrc })'), '旅人头像槽位挂载必须使用 mountedSrc。');
assert(albumPanel.includes('挂载NPC头像图片(prev, { npcId: params.targetId, slot: mapImageSlotToNpcAvatarSlot(params.slot), src: mountedSrc'), 'NPC 头像槽位挂载必须使用 mountedSrc。');
assert(albumPanel.includes('if (rawAvatar.trim().startsWith(\'asset:\')) return []'), '旅人当前 asset 引用头像不能再作为内置候选循环挂载。');
assert(albumPanel.includes('src: 解析相册资源引用(album, 读取NPC头像(npc, \'档案\'))'), '相册成品库 NPC 档案头像必须解析 asset 引用。');
assert(albumPanel.includes('src: 解析相册资源引用(album, traveler.图像档案?.头像 || traveler.头像 || undefined)'), '相册成品库旅人档案头像必须解析 asset 引用。');
