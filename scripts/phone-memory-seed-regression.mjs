...
assert(phoneService.includes('dedupePhoneReply'), 'phone replies must be deduped before landing.');
...
assert(phoneService.includes('buildNonRepeatingPhoneFallback'), 'phone replies must have a non-repeating fallback when all candidates duplicate history.');
assert(phoneService.includes("const messageLimit = ctx.chat.type === 'group' ? 20 : 6"), 'group phone replies must have a service-level maximum of 20 messages.');
...
assert(phoneCot.includes('至少出现 3 位不同发言者'), 'phone CoT must require at least three group speakers.');
assert(!phoneCot.includes('整个群聊总量 6-15 条'), 'phone CoT must not keep the retired 6-15 group-chat rule.');
assert(phoneWorldbook.includes('整个群聊本轮总回复量必须为 12-20 条'), 'phone worldbook must match the runtime group-chat 12-20 message rule.');
assert(phoneWorldbook.includes('至少出现 3 位不同发言者'), 'phone worldbook must require at least three group speakers.');
assert(!phoneWorldbook.includes('整个群聊本轮总回复量控制在 6-15 条'), 'phone worldbook must not keep the retired 6-15 group-chat rule.');
