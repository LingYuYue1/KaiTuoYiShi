...
assert(saveModal.includes('const allVisibleSaves = useMemo(() => visibleSaves.filter((s) => s.type !== \'auto\'), [visibleSaves])'), '读档弹窗全部计数必须排除自动存档，并过滤删除中的节点。');
...
assert(storageManager.includes('const allVisibleSaves = useMemo(() => visibleSaves.filter((s) => s.type !== \'auto\'), [visibleSaves])'), '设置存储管理全部计数必须排除自动存档，并过滤删除中的节点。');
