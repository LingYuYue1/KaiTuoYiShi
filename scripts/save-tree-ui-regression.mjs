...
assert(saveModal.includes('const allTreeGroups = useMemo(() => buildSaveTreeGroups(saves), [saves])'), '读档弹窗必须先用完整存档列表建立树。');
assert(saveModal.includes('buildVisibleSaveTreeGroup(group, tab)'), '读档弹窗必须筛选树内可见节点，不能用筛选节点重建树。');
assert(saveModal.includes("if (tab === 'all') return save.type !== 'auto'"), '读档弹窗的全部视图不得显示自动存档。');
assert(saveModal.includes('const allVisibleSaves = useMemo(() => saves.filter((s) => s.type !== \'auto\'), [saves])'), '读档弹窗全部计数必须排除自动存档。');
...
assert(saveModal.includes('md:grid-cols-[320px_minmax(0,1fr)_270px]') && saveModal.includes("borderLeft: '1px solid rgba(142,215,255,0.18)'"), '读档弹窗必须把存档树列表放在右侧独立栏。');
...
assert(!saveModal.includes('buildSaveTreeGroups(visibleSaves)'), '读档弹窗不能用筛选后的节点列表重建树。');
...
assert(saveModal.includes('kaituo-options-scroll grid w-full flex-shrink-0') && saveModal.includes('md:overflow-y-auto'), '读档弹窗左侧控制栏内容过高时必须独立滚动。');
assert(saveModal.includes('手动节点最多保留 6 个') && saveModal.includes('自动存档最多保留 6 棵树'), '读档弹窗存档策略必须提示手动节点与自动树上限。');
assert(saveModal.includes('保护 / 导入存档不计入手动上限'), '读档弹窗必须提示保护/导入存档不计入手动上限。');
assert(saveModal.includes('kaituo-options-scroll relative min-h-0 flex-1 overflow-y-auto'), '读档弹窗中间存档详情区必须使用同风格内部滚动。');
...
assert(saveModal.includes('linear-gradient(135deg, #8ed7ff, #5b99ff)'), '读档弹窗主按钮和选中态必须使用 A 方案蓝色渐变。');
assert(saveModal.includes('linear-gradient(#8ed7ff, rgba(142,215,255,0.08))'), '读档弹窗必须使用时间线树状节点视觉。');

assert(storageManager.includes('const allTreeGroups = useMemo(() => buildSaveTreeGroups(saves), [saves])'), '设置存储管理必须先用完整存档列表建立树。');
assert(storageManager.includes('buildVisibleSaveTreeGroup(group, filter)'), '设置存储管理必须筛选树内可见节点，不能用筛选节点重建树。');
assert(storageManager.includes("if (filter === 'all') return save.type !== 'auto'"), '设置存储管理的全部视图不得显示自动存档。');
assert(storageManager.includes('const allVisibleSaves = useMemo(() => saves.filter((s) => s.type !== \'auto\'), [saves])'), '设置存储管理全部计数必须排除自动存档。');
...
assert(!storageManager.includes('buildSaveTreeGroups(visible)'), '设置存储管理不能用筛选后的节点列表重建树。');
assert(!storageManager.includes('visible.map((save)'), '设置存储管理不能继续直接平铺 visible。');
assert(storageManager.includes('linear-gradient(135deg, #8ed7ff, #5b99ff)'), '设置存储管理必须同步新版蓝色按钮风格。');
